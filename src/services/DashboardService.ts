...
export class DashboardService {
  ...
  static async getDashboardStats() {
    ...
    const depositAggregate = await prisma.depositItem.aggregate({
      where: { deletedAt: null },
      _sum: { amount: true }
    });
    ...
    // Calculate last 6 months collection vs expense comparison
    const chartData = [];
    const now = new Date();
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthlyDepAggregate = await prisma.depositItem.aggregate({
        where: {
          deletedAt: null,
          deposit: {
      ...
      const monthlyExpAggregate = await prisma.expense.aggregate({
        where: {
          status: "APPROVED",
          deletedAt: null,
